...
public class DecideShipPlacementTester {
	...
	public void testDecideShipSimple() {
		...
		for (int i = 0; i < 10; ++i) {
			Ship expected = new Ship(Integer.parseInt(input[3*i]), Integer.parseInt(input[3*i + 1]), 1, Boolean.parseBoolean(input[3*i + 2]));
	...
	public void testDecideShipInvalidSimple() {
		...
		for (int i = 0; i < 10; ++i) {
			...
			if (i < 9) {
				...
				assertNotEquals("Given a repeat ship, you did not continue asking until a valid one was found", expected, shouldBeValid);
				ap.setCurArg(3*(i+1));
			}
		}
	}
