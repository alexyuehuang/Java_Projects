...
public class IsValidShipToAddTester {
	...
	public void testIsValidShipSimple() {
		...
		for (int i = 0; i < 10; ++i) {
			Ship s = new Ship(Integer.parseInt(input[3*i]), Integer.parseInt(input[3*i + 1]), 1, Boolean.parseBoolean(input[3*i + 2]));
	...
	public void testDecideShipInvalidComplex() {
		...
		boolean[] oTest = new boolean[] {true, true, false, true, false, true, true, false, false, true, false, false, true, true};
		...
		assertTrue("We should be able to add this ship", p.isValidShipToAdd(new Ship(xTest[xTest.length - 1], yTest[yTest.length - 1], 3, oTest[oTest.length - 1])));
	}
