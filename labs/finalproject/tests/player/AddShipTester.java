...
public class AddShipTester {
	...
	public void testInvalidAddShipTooLongVert() {
		assertFalse("Ships that are longer than the board should not be accepted", p.addShip(new Ship(0, 0, 20, false)));
	}
	...
	public void testInvalidAddShipTwoShipsOverlap() {
		...
		assertFalse("Do not accept a ship that overlaps with another existing ship", p.addShip(new Ship(1, 1, 1, true)));
		assertFalse("Do not accept a ship that overlaps with another existing ship", p.addShip(new Ship(1, 0, 2, false)));
	}
