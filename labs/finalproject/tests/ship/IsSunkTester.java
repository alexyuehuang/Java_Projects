...
public class IsSunkTester {
	...
	public void testTwentyLengthShipVert() {
		...
		s.isHit(22, 3); //make sure they didn't confuse vertical and horizontal
		assertFalse("We missed, this ship can't be sunk -- you may have confused vertical and horizontal", s.isSunk());
		for(int i = 3; i < 22; i++) {
			s.isHit(3, i); //hit
			assertFalse("We hit the ship at (3, " + i + ", but that's only part of it, it can't be sunk", s.isSunk());
		}
		s.isHit(3, 22); //sink
		assertTrue("We hit the final piece of the ship at (3, 22), it must be sunk", s.isSunk());
		s.isHit(1, 3); //make sure ship is still sunk
		assertTrue("We missed, but the ship should still be sunk", s.isSunk());
		s.isHit(2, 6);
		assertTrue("We missed, but the ship should still be sunk", s.isSunk());
	}
	...
	public void testTwentyLengthShipHoriz() {
		...
		s.isHit(2, 21); //make sure they didn't confuse vertical and horizontal
		assertFalse("We missed, this ship can't be sunk -- you may have confused vertical and horizontal", s.isSunk());
		for(int i = 2; i < 21; i++) {
			s.isHit(i, 2); //hit
